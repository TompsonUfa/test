...
$result = $mysqli->query("SELECT COUNT('test_id') as total FROM `tests`");
$row = mysqli_fetch_assoc($result);
$total = $row['total'];
$resultpage = ceil($total / $limit);

if ($test === "private"){
   $sql = "SELECT t.*,u.user_login,u.user_avatar,u.user_name,u.user_surname,u.user_midname FROM `tests` AS t
   JOIN `users` AS u on t.test_author = u.user_id WHERE u.user_id = $userId LIMIT $limit";
} else if ($test === "public") {
  $sql = "SELECT t.*,u.user_login,u.user_avatar,u.user_name,u.user_surname,u.user_midname FROM `tests` 
  AS t JOIN `users` AS u on t.test_author = u.user_id LIMIT $limit";
}
      ...
      <div class="cart__content">
        ...
        if ($test === "private"){
          echo '
            ...
            <a href="test?test='.$testId.'" class="btn me-2"> Редактировать </a>
            <a href="test?test='.$testId.'" class="btn btn-delete"> Удалить </a>
            </div>';
        } else if ($test === "public") {
          echo '
